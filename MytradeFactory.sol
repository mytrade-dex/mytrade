...
contract MytradePair is IMytradePair, MytradeERC20 {
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'MytradeSwap: TRANSFER_FAILED');
    }
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external onlyRouter lock {
        ...
        {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
            ...
            require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000 ** 2), 'MytradeSwap: K');
        }
